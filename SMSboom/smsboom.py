...
def safe_input(prompt: str):
    """Handle input safely in both script and frozen executable modes"""
    if getattr(sys, 'frozen', False):
        print(prompt)
        return ""  # Return default or empty value in frozen mode
    else:
        return input(prompt)
...
@click.command()
def update(apiS=None):
    """从 github 获取最新接口"""
    apiList = ['https://hk1.monika.love/OpenEthan/SMSBoom/master/', 'https://docs.superlinkstudio.top/File/smsboom/']
    
    if apiS is None:
        apiS = safe_input("选择更新的apiS接口[0.自带  1.SuperLinkStudioCloud]:")

    try:
        apiS = int(apiS)  # Ensure it's an integer
        ...
        GETAPI_json_url = f"{apiList[0]}GETAPI.json"
        API_json_url = f"{apiList[0]}api.json"
